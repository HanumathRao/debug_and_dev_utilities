...
#Delete previous output and leak report
sudo rm -f $MEMDUMP_OUTPUT
sudo touch $MEMDUMP_OUTPUT
sudo rm -f $LEAK_REPORT
python_script_full_path="./${MEMDUMP_PYTHON}"
gdb -batch -ex "source ${python_script_full_path}" -ex 'memdump' -ex 'r' ${DEBUGEE}
python ${MEMLEAK_PYTHON} ${MEMDUMP_OUTPUT} ${LEAK_REPORT}
echo
write_message "To see the output , type and enter : cat ${LEAK_REPORT}"