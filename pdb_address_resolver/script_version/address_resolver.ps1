param([string]$PORTABLE_EXECUTABLE_NAME="", [string]$HEX_ADDRESS="")
...
try
{
    Write-Host ( [AddressResolver]::getMethodNameFromAddress($PORTABLE_EXECUTABLE_NAME, $HEX_ADDRESS) )
}
catch
{
    $e = $_.Exception
	...
	Write-Host ""
	Write-Host "Usage : address_resolver.bat portable_executable_file_name hex_address"
	Write-Host ""
    exit 1
}
exit 0